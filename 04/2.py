...
def most_frequently_asleep(input):
  ...
  sleepiest_guard = max(timetable, key=(lambda x: max(timetable[x])))
...
assert most_frequently_asleep("""[1518-11-01 00:00] Guard #10 begins shift
...
if len(sys.argv) > 1:
  print(most_frequently_asleep(sys.argv[1]))
